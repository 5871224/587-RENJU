...
$latestRunUsesRenjuFilter = $latestRun !== null
    && str_contains((string)($latestRun['message'] ?? ''), 'category=1');
...
<main class="rn-main">
    ...
    <?php if ($totalRows > 0 && !$latestRunUsesRenjuFilter): ?>
        <div class="rn-warning"><strong>目前 RENJUNET_ELO 是舊條件的結果。</strong> 上一次重算尚未套用 RENJUNET_RULE.category=1 篩選，可能包含 Gomoku 等非 Renju 比賽；請執行一次「完整重算 RENJUNET Elo」。</div>
    <?php endif; ?>

    <div class="rn-notice">
        <strong>重算範圍：</strong>只納入 rated=1 且 RENJUNET_RULE.category=1 的 Renju 比賽，目前會排除 <?= number_format($excludedNonRenjuTournaments) ?> 場 rated 但非 Renju 的比賽。賽事依結束日期排序，若無結束日期則使用開始日期，再無則使用 year/month；同日以 RenjuNet tournament id 排序。重算成功前不會清除現有 RENJUNET_ELO。
    </div>
