...
$ratedTournaments = (int)$MYSQL->query('SELECT COUNT(*) FROM `RENJUNET_TOURNAMENT` WHERE `rated`=1')->fetchColumn();
$ratedGames = (int)$MYSQL->query(
    'SELECT COUNT(*) FROM `RENJUNET_GAME` G JOIN `RENJUNET_TOURNAMENT` T ON T.`id`=G.`tournament_id` WHERE T.`rated`=1'
)->fetchColumn();
...
<main class="rn-main">
    <section class="rn-hero">
        <div>
            ...
            <p>只計算 RENJUNET_TOURNAMENT.rated = 1；完全沿用目前台灣排名演算法，每位棋手第一次進入計算時初始分固定為 1850。</p>
        </div>
        <div class="rn-badge">完整重建 · 初始分 1850</div>
    </section>
    ...
    <div class="rn-notice">
        <strong>重算方式：</strong>賽事依結束日期排序，若無結束日期則使用開始日期，再無則使用 year/month；同日以 RenjuNet tournament id 排序。每場比賽內所有棋手都使用該場開始前的分數一起計算。重算成功前不會清除現有 RENJUNET_ELO，因此中途失敗仍保留上一次完整結果。
    </div>

    <section class="rn-grid">
        <div class="rn-card"><div class="label">Rated 比賽</div><div class="value"><?= number_format($ratedTournaments) ?></div></div>
        <div class="rn-card"><div class="label">Rated 對局</div><div class="value"><?= number_format($ratedGames) ?></div></div>
    ...
    <section class="rn-panel">
        <div class="rn-head">
            <div><h2>完整重算</h2><div class="rn-sub">歷史成績修正後，可直接重新建立全部 RenjuNet Elo。</div></div>
            <form method="post" onsubmit="return confirm('確定要完整重算所有 rated=1 的 RenjuNet 比賽嗎？成功後會整批取代目前 RENJUNET_ELO。')">
    ...
    <section class="rn-panel">
        <div class="rn-head">
            <div><h2>目前重算排名</h2><div class="rn-sub">每位棋手取 RENJUNET_ELO 最後一場比賽的 rating_after。</div></div>
            <form class="rn-tools" method="get">
                ...
                <?php if ($q !== ''): ?><a class="btn" href="?">清除</a><?php endif; ?>
            </form>
        </div>
        <?php if ($ranking): ?>
        <div class="rn-table-wrap"><table class="rn-table"><thead><tr><th>#</th><th>棋手 ID</th><th>棋手</th><th>國家</th><th>績分</th><th>局數</th><th>勝</th><th>和</th><th>負</th><th>最後比賽日</th><th>最後比賽</th></tr></thead><tbody>
        ...
        <?php else: ?><div class="rn-sub" style="padding:18px">目前沒有可顯示的重算結果。</div><?php endif; ?>
    </section>
